            ...
            for x in data:
                for k in x.keys():
                    if(k == 'audio' or k == 'synth_audio'):
                        ...
                        data = x[k]
                        concat_dim = 0 #changed from 3
                    print('shape of the data in results logger: {}, key: {}', data.shape, k)
...
class Trainer():
    def __init__(self,loaders,
                hyperparams:Hyperparams,device, training_logs_path='./logs/'):
        self.loaders = loaders
        self.hp = hyperparams
        self.training_logs_path = training_logs_path
    ...
    def train_step(self,model):
        ...
        for x in self.loaders['train']:
            ...
            #uncomment for debugging input shape
    ...
    def run(self,model,mode='train',resume_epoch=None):
        ...
        if(mode == 'train'):
            ...
            self.train_step_counter = 0
            self.writer = SummaryWriter(path.join(self.training_logs_path), flush_secs=20)
    ...
    def checkpoint(self,e):
        ...
        if(e==-1):
            torch.save(
                ...
                path.join(self.training_logs_path, "state_best.pth"),
            )
            return

        torch.save(
            self.model.state_dict(),
            path.join(self.training_logs_path, f"state_{e:06d}.pth"),
        )
        ...
        if(self.modelqueue.qsize()>self.hp.n_store_best):
            ...
            os.remove(
                path.join(self.training_logs_path, f"state_{delete_model_epoch:06d}.pth")
            )
        return
        ...
        if(self.device=='cpu'):
            if(e==-1):
                ...
                model.load_state_dict(
                    torch.load(
                        path.join(self.training_logs_path, f"state_best.pth"),map_location=torch.device('cpu')
                ))
            elif(e!=0):
                print("[INFO] Trainer.load_checkpoint on CPU: {}".format(e))
                model.load_state_dict(
                    torch.load(
                        path.join(self.training_logs_path, f"state_{e:06d}.pth"),map_location=torch.device('cpu')
                ))
        else:
            if(e==-1):
                ...
                model.load_state_dict(
                    torch.load(
                        path.join(self.training_logs_path, f"state_best.pth")
                ))
            elif(e!=0):
    ...
    def test(self,model):
        ...
        for k in self.loaders.keys():
            ...
            logger = ResultsLogger(f'{k}',
                                    data_dir=self.training_logs_path,
                                    sr=model.get_sr())
